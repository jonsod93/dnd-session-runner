    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {/* Name */}
          <div>
            <label className="block text-sm text-[#9a9894] mb-1.5">Name</label>
            <input
              type="text"
              value={name}
          ...
          {/* Icon */}
          <div>
            <label className="block text-sm text-[#9a9894] mb-1.5">Icon</label>
          ...
          {/* Color */}
          <div>
            <label className="block text-sm text-[#9a9894] mb-1.5">Color</label>
          ...
          {/* Notion link */}
          <div>
            <label className="block text-sm text-[#9a9894] mb-1.5">Notion Page</label>
            {notionPageId ? (
              ...
              <>
                <button
                  ...
                  className="text-sm text-[#9a9894] hover:text-gold-400 border border-white/[0.1] hover:border-gold-400/40 rounded px-2.5 py-1.5 transition-colors"
                >
                ...
                {showSearch && (
                  <div className="mt-2">
                    ...
                    <div className="mt-1.5 max-h-40 overflow-y-auto bg-[#1e1e1e] border border-white/[0.08] rounded">
                      {searching && (
                        <p className="text-xs text-[#9a9894] px-3 py-2 italic">Searching…</p>
                      )}
                      {!searching && searchResults.length === 0 && searchQuery.trim() && (
                        <p className="text-xs text-[#9a9894] px-3 py-2">No results</p>
                      )}
                      {searchResults.map((r) => (
                        <button
                          key={r.id}
                        ...
                        >
                          <div className="text-sm text-[#e6e6e6]">{r.title}</div>
                          {r.types.length > 0 && (
                            <div className="text-xs text-[#9a9894] mt-0.5">{r.types.join(', ')}</div>
                          )}
                        </button>
                      ))}
                    </div>
          ...
          <div>
            <label className="block text-sm text-[#9a9894] mb-1">Coordinates</label>
